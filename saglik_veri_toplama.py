...
VERI_KLASORU = os.path.join(BASE_DIR, "veriler", "ham_veri")
...
try:
    saglik_df = pd.read_csv(f"{VERI_KLASORU}/ilce_saglik_birlesik.csv", encoding='utf-8')
    print(f"✅ Sağlık verileri: {len(saglik_df)} ilçe")
except FileNotFoundError:
    print("❌ HATA: ilce_saglik_birlesik.csv bulunamadı!")
    print("   Önce saglik_veri_toplama.py çalıştırın")
    exit(1)
...
for idx, row in tum_ilceler.iterrows():
    ...
    ilce_dict = {
        ...
        "universite_hastanesi": int(row['Universite_Hastanesi']),
...
print("   python ai_saglik_oneri_sistemi.py")
print("=" * 70)